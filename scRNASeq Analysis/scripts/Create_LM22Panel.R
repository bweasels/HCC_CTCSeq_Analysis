...
library(pheatmap)
date <- Sys.Date()
setwd('/Dropbox (Partners HealthCare)/Weekly Analyses/09-23-19/Regenerate Cell Panel LM22/')

#load Cibersort's 22 sample panel
cibersortLM22Counts <- read.table('/Dropbox (Partners HealthCare)/Dropbox (Partners HealthCare)/Weekly Analyses/08-20-19/CellTypePanel/LM22-ref-sample.txt', sep = '\t', row.names = 1)
cibersortLM22Ref <- read.table('/Dropbox (Partners HealthCare)/Dropbox (Partners HealthCare)/Weekly Analyses/08-20-19/CellTypePanel/LM22-classes.txt', sep = '\t', row.names = 1)
...
for(i in 1:length(signature)){
  selected <- FoldChange[FoldChange[,i]>8, i]
